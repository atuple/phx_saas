defmodule PhxSaas.TenantActionsTest do
  use ExUnit.Case

  alias PhxSaas.Note
  alias PhxSaas.TestPostgresRepo
  ...
  test ".migrate_tenant/4 migrates the tenant forward by default" do
    ...
    assert_creates_notes_table fn ->
      {status, prefix, versions} = PhxSaas.migrate_tenant(@repo, @tenant_id)
  ...
  test ".migrate_tenant/4 returns an error tuple when it fails" do
    ...
    force_migration_failure fn(expected_postgres_error) ->
      {status, prefix, error_message} = PhxSaas.migrate_tenant(@repo, @tenant_id)
  ...
  test ".migrate_tenant/4 can rollback and return metadata" do
    ...
    assert_drops_notes_table fn ->
      {status, prefix, versions} =
        PhxSaas.migrate_tenant(@repo, @tenant_id, :down, to: @migration_version)
  ...
  defp create_and_migrate_tenant do
    PhxSaas.new_tenant(@repo, @tenant_id)
  end

  defp create_tenant_schema do
    PhxSaas.TenantActions.create_schema(@repo, @tenant_id)
  end

  defp find_tenant_notes do
    PhxSaas.all(@repo, Note, @tenant_id)
  end
