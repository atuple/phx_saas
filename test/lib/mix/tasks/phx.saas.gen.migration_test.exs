defmodule Mix.Tasks.PhxSaas.Gen.MigrationTest do
  use ExUnit.Case, async: true

  import Support.FileHelpers
  import Mix.Tasks.PhxSaas.Gen.Migration, only: [run: 1]

  tmp_path = Path.join(tmp_path(), inspect(PhxSaas.Gen.Migration))
  ...
  defmodule Repo do
    ...
    def config do
      [priv: "tmp/#{inspect(PhxSaas.Gen.Migration)}", otp_app: :PhxSaas]
    end
  end
  ...
  test "generates a new migration" do
    ...
    assert_file Path.join(@migrations_path, name), fn file ->
      assert file =~ "defmodule Mix.Tasks.PhxSaas.Gen.MigrationTest.Repo.TenantMigrations.MyMigration do"
